...
- агрегированный вызов по нескольким филиалам: `GET /api/v2/queues/aggregate`;
...
- декларативные programmable endpoints через `POST /api/v2/program/{endpointId}`.
...
- ingestion endpoint: `POST /api/v2/events/ingest`;
- inbound Kafka/DataBus listener для подчиненной шины СУО (`integration.eventing.kafka.enabled=true`);
- replay endpoint: `POST /api/v2/events/replay/{eventId}`;
- DLQ replay endpoint: `POST /api/v2/events/replay-dlq/{eventId}`;
- DLQ bulk replay endpoint: `POST /api/v2/events/replay-dlq?limit=N`;
- ops endpoints: `/api/v2/events/dlq/*`, `/api/v2/events/processed/*`, `/api/v2/events/stats`;
- outbox reliability endpoints: `GET /api/v2/events/outbox`, `GET /api/v2/events/outbox/{eventId}`, `POST /api/v2/events/outbox/flush?limit=N`;
- ops filters: `eventType/source/from/to/offset/limit/desc` для DLQ/processed выборок;
- maintenance endpoint: `POST /api/v2/events/maintenance/run`;
- snapshot endpoints: `GET /api/v2/events/export`, `POST /api/v2/events/import`;
- governance endpoints: `POST /api/v2/events/import/preview?clearBeforeImport=...&strictPolicies=...`, `POST /api/v2/events/import/validate?strictPolicies=...`, `POST /api/v2/events/import/analyze`, `GET /api/v2/events/capabilities`, `GET /api/v2/events/limits`, `GET /api/v2/events/version`;
...
- ITS import/export programmable handlers:
  - `POST /api/v2/program/templates/import/preview` (multipart: `archive`)
  - `POST /api/v2/program/templates/import` (multipart: `archive`, `parameterValues`, `replaceExisting`)
  - `POST /api/v2/program/templates/export` (JSON -> `*.its` archive download)
...
- Расширение reliability inbox/outbox:
  ...
  - endpoint восстановления зависших inbox-записей: `POST /api/v2/events/inbox/recover-stale`.
...
- Расширенный каталог коннекторов внешних шин/брокеров для GUI:
  - `GET /api/v2/program/connectors/catalog` и `GET /api/v2/program/connectors/broker-types` возвращают `supportedBrokerProfiles` с шаблонами параметров;
...
- CRM/МИС интеграция по идентификатору клиента:
  - добавлены endpoint-и `POST /api/v2/program/connectors/crm/identify-client`, `POST /api/v2/program/connectors/crm/medical-services`, `POST /api/v2/program/connectors/crm/prebooking`;
